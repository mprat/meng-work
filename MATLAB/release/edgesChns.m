...
for t=1:nTypes
  ...
  if(size(I,3)==1), cs='gray'; else cs='luv'; end; I=dollar_toolbox.channels.rgbConvert(I,cs);
  Ishrink=dollar_toolbox.channels.imResample(I,1/shrink); k=k+1; chns{k}=Ishrink;
  for i = 1:2, s=2^(i-1);
    if(s==shrink), I1=Ishrink; else I1=dollar_toolbox.channels.imResample(I,1/s); end
    I1 = dollar_toolbox.channels.convTri( I1, opts.grdSmooth );
    [M,O] = dollar_toolbox.channels.gradientMag( I1, 0, opts.normRad, .01 );
    H = dollar_toolbox.channels.gradientHist( M, O, max(1,shrink/s), opts.nOrients, 0 );
    k=k+1; chns{k}=dollar_toolbox.channels.imResample(M,s/shrink);
    k=k+1; chns{k}=dollar_toolbox.channels.imResample(H,max(1,s/shrink));
  end
end
...
chnsReg=dollar_toolbox.channels.convTri(chns,chnSm); chnsSim=dollar_toolbox.channels.convTri(chns,simSm);
